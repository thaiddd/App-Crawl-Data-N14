...
class Window(tk.Tk):
    def __init__(self):
        ...
        tk.Tk.__init__(self)
        #super().__init__()
        self.title('TEAM 14 - Python scraping')
    ...
    #     self.productList = []
    #     tk.Tk.__init__(self)
    #     self.title('Nhóm 14 - App Crawl Data')
    #     self.geometry(self.get_position(800, 500))
    #     self.resizable(False, False)
    #     self.myfont = ('Arial', '11', 'bold')
    #     self.initPage()
    #     self.initProgressBar()
    ...
    def initProgressBar(self):
        ...
        # )
        self.cancel_button = ttk.Button(self.win, text='Cancel', command=self.win.destroy)
        self.cancel_button.grid(column=0, row=1, padx=10, pady=10, sticky=tk.E)
    ...
    def create_tab1(self):
        ...
        # self.imgLogo = ImageTk.PhotoImage(Image.open('D:\Python-scraping-application-main\data\Img\logoshopee.png').resize((205, 205)))
        # self.lbLogo = tk.Label(self.tab1, image=self.imgLogo)

        try:
            self.imgLogo = ImageTk.PhotoImage(
                Image.open('D:\Python-scraping-application-main\data\Img\logoshopee.png').resize((205, 205)))
            self.lbLogo = tk.Label(self.tab1, image=self.imgLogo)
        except FileNotFoundError:
            print("Lỗi: Không tìm thấy logo Shopee. Tiếp tục mà không có logo.")
            self.lbLogo = tk.Label(self.tab1, text="Không tìm thấy logo Shopee")

        self.lbLogo.place(x=550, y=45)

        self.btnSearch = tk.Button(self.tab1, text='Lấy dữ liệu!', font=self.myfont, command= lambda: threading.Thread(target=accessToShopee, args=(self, self.eNumberOfPage.get(), self.eSearch.get())).start())
        self.lbCopyRight = tk.Label(self.tab1, text='Nhóm 14 - Python thầy Vũ Minh Mạnh',
    ...
    def create_tab2(self):
        ...
        self.btnSearch2 = tk.Button(self.tab2, text='Lấy ảnh!', font=self.myfont,
                                    command=lambda: threading.Thread(target=downloadImages, args=(
                                    self, self.eUrl.get(), './data/downloadedImages', self.eLimit.get())).start())
    ...
    #     maxsize = (800, 500)
    #     # self.img = Image.open('D:\Python-scraping-application-main\data\Img\Background.jpg')
    ...
    #         self.rootImg.pack()
    #     except FileNotFoundError:
    #         print("Lỗi: Không tìm thấy hình nền. Tiếp tục mà không có hình nền.")
    #         self.rootImg = tk.Label(self, text="Không tìm thấy hình nền")
    #         self.rootImg.pack()
    ...
    def initPage(self):
        print("Bắt đầu initPage")
        maxsize = (800, 500)
        try:
            self.img = Image.open('D:\Python-scraping-application-main\data\Img\Background.png')
            self.img = self.img.resize(maxsize)
            self.bg = ImageTk.PhotoImage(self.img)
            # Đặt hình nền làm background của cửa sổ chính
            self.rootImg = tk.Label(self, image=self.bg)
            self.rootImg.place(x=0, y=0, relwidth=1, relheight=1)  # Sử dụng place để đặt hình nền
        except FileNotFoundError:
            print("Lỗi: Không tìm thấy hình nền. Tiếp tục mà không có hình nền.")
            self.rootImg = tk.Label(self, text="Không tìm thấy hình nền")
            self.rootImg.pack(fill="both", expand=True)
        print("Đã thêm hình nền")

        self.btn_about_us = tk.Button(self, text='More info', bg='black',
                                      fg='white', width=9, height=2,
                                      font=('Arial', 10, 'bold'), command=self.accessToGithub)
        self.btn_about_us.pack(pady=2, anchor='e')

        self.tabControl = ttk.Notebook(self)
        print("Đã tạo ttk.Notebook")
        self.tab1 = self.create_tab1()
        print("Đã tạo tab 1")
        self.tab2 = self.create_tab2()
        print("Đã tạo tab 2")

        self.tabControl.add(self.tab1, text='Lấy dữ liệu từ Shopee')
        self.tabControl.add(self.tab2, text='Lấy dữ liệu từ Tiki')
        self.tabControl.pack(expand=1, fill="both")
        print("Đã thêm notebook vào giao diện")
